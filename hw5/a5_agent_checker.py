...
try:
	state = TTS_State.TTS_State(board)
	results = Player.parameterized_minimax(current_state=state, max_ply=2, alpha_beta = False, use_custom_static_eval_function=False)
	assert(results["CURRENT_STATE_STATIC_VAL"] == 1)
	assert(results["N_STATES_EXPANDED"] == 4)
	assert(results["N_STATIC_EVALS"] == 6)
	assert(results["N_CUTOFFS"] == 0)
except Exception:
